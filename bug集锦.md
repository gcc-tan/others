...
###C语言中的正数类型和转换陷阱
对于C语言中整数采用的是补码存储。这里简单介绍一下原码，反码，补码表示。以一个字节的数据为例：

+ 原码，采用最高为符号位，0表示正，1表示负。所以一个字节的正数表示的范围就是-127～-0， +0～+127。因为0有下面两种表示方法：
```
1000 0000 或者 0000 0000
```

+ 反码。就是除符号位外，将原码按位取反
+ 补码。在反码的基础上末尾加1。这个地方需要解释一下。在补码表示中正0与负0进行了统一，那么就多出了另外一个编码1000 0000可以表示其他数字。由于最高位为1,表示的是负数，那么从-127的地方,扩展一个。
```
-0的反码表示，1111 1111，然后加1，由于进位变成0000 0000
+0的反码表示，0111 1111,然后加1,由于进位变成0000 0000
```

**C语言中有符号数和无符号数进行运算（包括逻辑运算和算术运算）默认会将有符号数看成无符号数进行运算，其中算术运算默认返回无符号数，逻辑运算当然是返回0或1了。**


**当数据类型转换时，同时需要在不同数据大小，以及无符号和有符号之间转换时，C语言标准要求先进行数据大小的转换，之后再进行无符号和有符号之间的转换。将无符号数转换为更大的数据类型时, 只需简单地在开头添加0，这种运算称为0扩展。将有符号数转换为更大的数据类型需要执行符号扩展，规则是将符号位扩展至所需要的位数，采用符号扩展和0扩展的方式为了保证数值大小不会改变**
...
int main()
{
    ...
    unsigned int b = 16;
    if(a > b)
        cout<<"impossible negative bigger than positive"<<endl;
    return 0;
}
```
上面程序的运行结果是：
```
impossible negative bigger than positive
```
运行原因分析。有符号数字a与无符号数字b进行运算（逻辑运算），a会转换成unsigned int类型，那么a(32位)本来的补码表示为0xff ff ff ff，用unsigned int解释当然比b大
...
-2
```
运行原因分析。这个程序的运行结果看似是和之前的解释自相矛盾。但是，中间有一些知识点还没有讲解。

**因为在C语言中比int长度小（包括char, unsigned char, short，unsigned short）的整形数进行运算时需要将整形的数进行转换成int进行计算。这么做是为了效率考虑。一般来说int的长度都会被定义为机器处理效率最高的长度，在32位机器上，int一般是4个字节。**

有了这个知识，再来分析一下原因。-16(16bit 的补码为 1111 0000)采用符号扩展扩展成int，在内存中存储的是1111 1111 1111 0000（可以计算一下，还是-16）。后面的b扩展的结果同理，最终结果就是-2。


内容来自：

[有符号数和无符号数的转换及思考](https://blog.csdn.net/gatieme/article/details/52557546)

[C语言中无符号数和有符号数之间的运算](https://www.cnblogs.com/qingergege/p/7507533.html)
